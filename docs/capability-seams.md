...
| `ctx.sessionBrief` | `seam` | [`session-brief`](../packages/session/session-brief) | [`session-brief-llm`](../packages/session/session-brief-llm) | [`command-session-brief`](../packages/session/command-session-brief) | - | Owns provider registration, stable-revision generation, accepted brief projection, and automatic or explicit refresh coordination. |
